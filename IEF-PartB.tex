...
$\sqrt{s}$. The ratio $\PgUc/\PgUa$ has been measured by CMS, but not the \rpa (\PgUc), never directly measured to date. The latter is important to study the different effects of CNM effects, in particular nuclear absorption and comovers, on the ground and excited states.
...
PbPb collisions divided into centrality classes highlighted in different colours.
  Middle: $\Dz \to \K\pi$ in 70-90\% \pbpb in LHCb. Right: $\raa(\D)$ from ALICE.
  \label{fig:pbpblhcb}}
\end{figure}
...
The \Dz and \Jpsi analyses based on these data are currently under internal review and are expected to be published in the end of 2016. A clear \Dz peak is shown in Fig.~\ref{fig:pbpblhcb} (right). Since the regular LHCb tracking algorithms optimised for \pp with minimal changes have been used in reconstructing these data, the analyses have only address events up to centralities of about 60\% (see Fig.~\ref{fig:pbpblhcb}, left). This limit correspond to about 15\,000 clusters in the VELO, value for which the tracking performance starts to become inefficient.
